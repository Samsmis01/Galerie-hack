...
def start_serveo():
    ...
    try:
        ...
        if str(LOCAL_PORT) in port_check.stdout:
            print(f"{Colors.YELLOW}[!] Le port {LOCAL_PORT} est déjà utilisé{Colors.END}")
            return False

        # Lance le tunnel Serveo
        process = subprocess.Popen(
            ["ssh", "-o", "StrictHostKeyChecking=no", "-R", f"80:localhost:{LOCAL_PORT}", "serveo.net"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True
        )
        ...
        url = None
        for _ in range(15):  # Augmenté le timeout à 15 secondes
            line = process.stderr.readline()
            if "Forwarding" in line:
                url = line.strip().split()[-1]
                print(f"\n{Colors.GREEN}[✓] Tunnel Serveo actif!{Colors.END}")
                print(f"{Colors.BLUE}[→] Lien généré: https://{url}{Colors.END}")
                print(f"\n{Colors.YELLOW}[!] Gardez ce terminal ouvert pendant l'utilisation{Colors.END}")
                return True
            sleep(1)

        if not url:
            print(f"{Colors.RED}[!] Échec de connexion à Serveo{Colors.END}")
            print(f"{Colors.YELLOW}[*] Essayez ces solutions:{Colors.END}")
            print("1. Vérifiez votre connexion Internet")
            print("2. Réessayez dans 2-3 minutes")
            print("3. Utilisez un réseau différent si possible")
            return False
    ...
    while True:
        try:
            ...
            if choice == "1":
                ...
                print(f"{Colors.BLUE}[→] Lien direct: {TELEGRAM_LINK}{Colors.END}")
                ...
                if start_serveo():
                    print(f"{Colors.GREEN}[✓] Prêt à recevoir des connexions{Colors.END}")
                else:
                    print(f"{Colors.RED}[!] Impossible d'établir le tunnel{Colors.END}")
